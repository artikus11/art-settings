...
* Перемещение подменю после регистрации: `menu.position` в конфиге теперь (кроме передачи в
  `add_submenu_page`/`add_menu_page`) сортирует пункт в `$submenu` в конце хука `admin_menu`:
  * `'first'` — пункт в начало списка подменю;
  * `'last'` — пункт в конец (переживает подменю, добавленные другими плагинами позже);
...
* PHPUnit-тесты перекладки: конец/начало/индекс, отсутствующий пункт, `last`/`first`/`int`.
